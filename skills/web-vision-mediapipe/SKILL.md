...
- **Verify it with a real camera - `--camera` plays a picture as the webcam.** `gipity page eval <url> --camera ./fist.jpg --wait 6000 "<read what the app rendered>"` (also on `page screenshot`/`inspect`) runs the app's actual camera → inference → `onResult` path against that image, so you check the app's own output, not a hand-fed fake. No hand photo around? `gipity generate image "photo of a closed fist, plain background"`. Don't ship debug hooks on `window` to test this - see [app-debugging](https://docs.gipity.ai/skills/app-debugging.html).
