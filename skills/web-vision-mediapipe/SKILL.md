...
- **Verify the logic without a camera - the camera path can't be tested headlessly.** A headless `gipity page inspect`/`eval` browser has no webcam, so driving the UI reaches only the "camera unavailable" gate; the inference loop and your `onResult` handler never run. Keep the logic downstream of `onResult` (gesture→action mapping, round/score state) in plain functions exposed on `window`, then drive them from `page eval` with a synthetic result (`{ gestures:[[{ categoryName:'Closed_Fist', score:1 }]] }`) to exercise the real feature. Confirming only the camera gate verifies nothing the user asked for - see [app-debugging](https://docs.gipity.ai/skills/app-debugging.html).
