...
- **Project hooks** (Claude Code and Grok) - keep a linked Gipity project in sync without the model having to remember to: every file the agent writes is pushed to your cloud workspace (`hooks/scripts/sync-push.cjs`), and cloud-side changes are pulled before each turn (`sync-pull.cjs`). In Claude Code, terminal sessions in the project are also mirrored to the web CLI (`capture.cjs`) - however you launch them (`gipity claude` or plain `claude`; on a machine paired via `gipity login`). Opt a project out of session recording with `gipity init --no-capture` (writes `captureHooks: false` to `.gipity.json`), or set `GIPITY_CAPTURE=off` for a one-off unrecorded session. Every hook checks for a `.gipity.json` in the working directory first and exits instantly outside Gipity projects - nothing runs, syncs, or phones home in your other repos. Uninstalling or disabling the plugin removes all hooks in one step.
